...
def update_or_insert_yf_daily_data(cur, ticker, data_list):
    for row in data_list:
        check_query = "SELECT COUNT(*) FROM yfDailyPrice WHERE Ticker = %s::text AND  Date = %s::date;"
        cur.execute(check_query, (ticker, row["Date"]))
        count = cur.fetchone()[0]

        if count > 0:
            # Data already exists, you can choose to update it or skip
            pass
        else:
            insert_query = """
            INSERT INTO yfDailyPrice (Ticker, Date, Open, High, Low, Close, Volume, Dividends, "Stock Splits")
...
def update_or_insert_yf_intraday_data(cur, ticker, rows):
    for row in rows:
        check_query = "SELECT COUNT(*) FROM yfIntradayPrice WHERE Ticker = %s::text AND Date = %s::date;"
        cur.execute(check_query, (ticker, row["Date"]))
        count = cur.fetchone()[0]
        ...
        if count > 0:
            ...
            insert_query = """
            INSERT INTO yfIntradayPrice (Ticker, Date, Open, Close, Volume, Delta, Hourly_RoC)
            VALUES (%s, %s,%s, %s, %s, %s, %s) ON CONFLICT (Ticker, Date) DO NOTHING;
            """
            cur.execute(insert_query, (
                ticker,
                row['Date'], 
...
def create_tradelog_if_not_exists(cur):
    create_tradelog_query = """
    ...
    """
    cur.execute(create_tradelog_query)            

    
def update_or_insert_tradelog_data(cur, ticker, row):  # pass in portfolio and strategy
    check_query = "SELECT COUNT(*) FROM tradelog WHERE Ticker = %s::text;"
    cur.execute(check_query, (ticker,))
    ...
    if count > 0:
        ...
        insert_query = """
        INSERT INTO tradelog (Strategy, Ticker, Date, Action, Price, Quantity, PnL)        VALUES 
        (%s, %s,%s, %s, %s, %s, %s);
        """
        cur.execute(insert_query, (
            ticker,
...
# # holdings
def create_holding_if_not_exists(cur):
    ...
    CREATE TABLE IF NOT EXISTS holding (
        ...
        Quantity VARCHAR(50));
    """
    cur.execute(create_holding_query)

def update_or_insert_holding_data(cur, ticker, row):
    insert_query = """
    INSERT INTO holding (Strategy, Ticker, Date, Price, Quantity) VALUES 
    (%s, %s, %s, %s, %s);
    """
    cur.execute(insert_query, (
        row['Strategy'], 
...
def create_strategy_summary_if_not_exists(cur):
    create_strategy_summary_query = """
    CREATE TABLE IF NOT EXISTS strategy_summary (
                ...
                end_date DATE,
                final_value NUMERIC,
                PRIMARY KEY (class_name, cash_used, start_date, end_date)

        )
    """
    cur.execute(create_strategy_summary_query)
    
def create_strategy_summary_LONG_if_not_exists(cur):
    create_strategy_summary_LONG_query = """
    CREATE TABLE IF NOT EXISTS strategy_summary_LONG (
                class_name TEXT,
                cash_used NUMERIC,  
                start_date DATE,
                end_date DATE,
                final_value NUMERIC,
                adjusted_sortino NUMERIC,
                autocorr_penalty NUMERIC,
                avg_loss NUMERIC, 
                avg_return NUMERIC, 
                avg_win NUMERIC,
                best NUMERIC, 
                cagr NUMERIC, 
                calmar NUMERIC, 
                common_sense_ratio NUMERIC, 
                comp NUMERIC, 
                conditional_value_at_risk NUMERIC, 
                consecutive_losses NUMERIC, 
                consecutive_wins NUMERIC, 
                cpc_index NUMERIC, 
                cvar NUMERIC,
                expected_return NUMERIC,
                expected_shortfall NUMERIC, 
                exposure NUMERIC, 
                gain_to_pain_ratio NUMERIC,
                geometric_mean NUMERIC, 
                ghpr NUMERIC, 
                kelly_criterion NUMERIC, 
                kurtosis NUMERIC, 
                max_drawdown NUMERIC, 
                outlier_loss_ratio NUMERIC, 
                outlier_win_ratio NUMERIC,
                payoff_ratio NUMERIC, 
                probabilistic_adjusted_sortino_ratio NUMERIC, 
                probabilistic_ratio NUMERIC, 
                probabilistic_sharpe_ratio NUMERIC, 
                probabilistic_sortino_ratio NUMERIC,
                ...
                serenity_index NUMERIC, 
                sharpe NUMERIC, 
                skew NUMERIC, 
                smart_sharpe NUMERIC, 
                smart_sortino NUMERIC, 
                sortino NUMERIC, 
                tail_ratio NUMERIC, 
                ulcer_index NUMERIC, 
                ulcer_performance_index NUMERIC, 
                upi NUMERIC, 
    ...
    """
    cur.execute(create_strategy_summary_LONG_query)
...
def update_or_insert_strategy_summary(cur, class_name, cash_used, start_date, end_date, final_value):
    # Check if the entry already exists based on class_name and start_date
    cur.execute("SELECT * FROM strategy_summary WHERE class_name = %s AND start_date = %s", (class_name, start_date,))
    existing_row = cur.fetchone()

    if existing_row:
        # If it exists, update the entry
        cur.execute("UPDATE strategy_summary SET final_value = %s WHERE cash_used = %s AND start_date = %s AND end_date = %s AND class_name = %s",
                    (final_value, cash_used, start_date, end_date, class_name))
    else:
        # If it doesn't exist, insert a new entry
        cur.execute("INSERT INTO strategy_summary (class_name, cash_used, start_date, end_date, final_value) VALUES (%s, %s, %s, %s, %s)",
                    (class_name, cash_used, start_date, end_date, final_value))

def update_or_insert_strategy_summary_LONG(cur, class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win,best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst):
    cur.execute("SELECT * FROM strategy_summary_LONG WHERE class_name = %s AND start_date = %s", (class_name, start_date,))
    existing_row = cur.fetchone()

    if existing_row:
        # If it exists, update the entry
        cur.execute("UPDATE strategy_summary_LONG SET final_value = %s, adjusted_sortino = %s, autocorr_penalty = %s, avg_loss = %s, avg_return = %s, avg_win = %s, best = %s, cagr = %s, calmar = %s, common_sense_ratio = %s, comp = %s, conditional_value_at_risk = %s, consecutive_losses = %s, consecutive_wins = %s, cpc_index = %s, cvar = %s, expected_return = %s, expected_shortfall = %s, exposure = %s, gain_to_pain_ratio = %s, geometric_mean = %s, ghpr = %s, kelly_criterion = %s, kurtosis = %s, max_drawdown = %s, outlier_loss_ratio = %s, outlier_win_ratio = %s, payoff_ratio = %s, probabilistic_adjusted_sortino_ratio = %s, probabilistic_ratio = %s, probabilistic_sharpe_ratio = %s, probabilistic_sortino_ratio = %s, profit_factor = %s, profit_ratio = %s, rar = %s, recovery_factor = %s, risk_of_ruin = %s, risk_return_ratio = %s, serenity_index = %s, sharpe = %s, skew = %s, smart_sharpe = %s, smart_sortino = %s, sortino = %s, tail_ratio = %s, ulcer_index = %s, ulcer_performance_index = %s, upi = %s, value_at_risk = %s, var = %s, volatility = %s, win_loss_ratio = %s, win_rate = %s, worst = %s WHERE cash_used = %s AND start_date = %s AND end_date = %s AND class_name = %s",
                    (final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win,best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst, cash_used, start_date, end_date, class_name))
    else:
        cur.execute("INSERT INTO strategy_summary_LONG(class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win,best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst) VALUES (%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s,%s,%s)",
                    (class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win, best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst))
...
#def update_or_insert_strategy_summary_LONG(cur, class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win,best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio): 
    # , outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor
    # , risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index
    # , ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst):
    # Check if the entry already exists based on class_name and start_date



def create_performance_if_not_exists(cur):
    ...
    CREATE TABLE IF NOT EXISTS performance (
                ...
                end_date DATE,
                final_value TEXT,
                adjusted_sortino TEXT,
                autocorr_penalty TEXT,
                avg_loss TEXT,
                avg_return TEXT,
                avg_win TEXT,
                PRIMARY KEY (class_name, cash_used, start_date, end_date)
    ...
    """
                ...
                # cagr TEXT,
                # calmar TEXT,
                # common_sense_ratio TEXT,
                # comp TEXT,
                # conditional_value_at_risk TEXT,
                # consecutive_losses TEXT,
                # consecutive_wins TEXT,
                # cpc_index TEXT,
                # cvar TEXT,
                ...
                # exposure TEXT,
                # gain_to_pain_ratio TEXT,
                # geometric_mean TEXT,
                # ghpr TEXT,
                # kelly_criterion TEXT,
                # kurtosis TEXT,
                # max_drawdown TEXT,
                # outlier_loss_ratio TEXT,
                
                # outlier_win_ratio TEXT,
                # payoff_ratio TEXT,
                # probabilistic_adjusted_sortino_ratio TEXT,
                # probabilistic_ratio TEXT,
                # probabilistic_sharpe_ratio TEXT,
                # probabilistic_sortino_ratio TEXT,
                # profit_factor TEXT,
                # profit_ratio TEXT,
                # rar TEXT,
...
def update_or_insert_performance_data(cur, class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win):#, best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst):
    # Check if the entry already exists based on class_name and start_date
    cur.execute("SELECT * FROM performance WHERE class_name = %s AND start_date = %s", (class_name, start_date,))
    existing_row = cur.fetchone()
    print(class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win)
    if existing_row:
        # If it exists, update the entry
        cur.execute("UPDATE performance SET final_value = %s, adjusted_sortino = %s, autocorr_penalty = %s, avg_loss = %s, avg_return = %s, avg_win = %s WHERE cash_used = %s AND start_date = %s AND end_date = %s AND class_name = %s ",#, best = %s, cagr = %s, calmar = %s, common_sense_ratio = %s, comp = %s, conditional_value_at_risk = %s, consecutive_losses = %s, consecutive_wins = %s, cpc_index = %s, cvar = %s, expected_return = %s, expected_shortfall = %s, exposure = %s, gain_to_pain_ratio = %s, geometric_mean = %s, ghpr = %s, kelly_criterion = %s, kurtosis = %s, max_drawdown = %s, outlier_loss_ratio = %s, outlier_win_ratio = %s, payoff_ratio = %s, probabilistic_adjusted_sortino_ratio = %s, probabilistic_ratio = %s, probabilistic_sharpe_ratio = %s, probabilistic_sortino_ratio = %s, profit_factor = %s, profit_ratio = %s, rar = %s, recovery_factor = %s, risk_of_ruin = %s, risk_return_ratio = %s, serenity_index = %s, sharpe = %s, skew = %s, smart_sharpe = %s, smart_sortino = %s, sortino = %s, tail_ratio = %s, ulcer_index = %s, ulcer_performance_index = %s, upi = %s, value_at_risk = %s, var = %s, volatility = %s, win_loss_ratio = %s, win_rate = %s, worst = %s WHERE class_name = %s AND cash_used = %s AND start_date = %s AND end_date = %s ",
                    (final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win, cash_used, start_date, end_date, class_name))# best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst, cash_used, start_date, end_date, class_name))
    else:
        # If it doesn't exist, insert a new entry
        cur.execute("INSERT INTO performance (class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", # best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (class_name, cash_used, start_date, end_date, final_value, adjusted_sortino, autocorr_penalty, avg_loss, avg_return, avg_win)) #, best, cagr, calmar, common_sense_ratio, comp, conditional_value_at_risk, consecutive_losses, consecutive_wins, cpc_index, cvar, expected_return, expected_shortfall, exposure, gain_to_pain_ratio, geometric_mean, ghpr, kelly_criterion, kurtosis, max_drawdown, outlier_loss_ratio, outlier_win_ratio, payoff_ratio, probabilistic_adjusted_sortino_ratio, probabilistic_ratio, probabilistic_sharpe_ratio, probabilistic_sortino_ratio, profit_factor, profit_ratio, rar, recovery_factor, risk_of_ruin, risk_return_ratio, serenity_index, sharpe, skew, smart_sharpe, smart_sortino, sortino, tail_ratio, ulcer_index, ulcer_performance_index, upi, value_at_risk, var, volatility, win_loss_ratio, win_rate, worst))
        
def create_backperformance_if_not_exists(cur):
    create_backperformance_query = """
    CREATE TABLE IF NOT EXISTS backperformance (
        Strategy TEXT,
        cash TEXT,  
        start_date DATE,
        end_date DATE,
        ...
        SharpeRatio TEXT,
        Calmar TEXT,
        PRIMARY KEY (Strategy, cash, start_date, end_date)
    )
    """
    cur.execute(create_backperformance_query)

def update_or_insert_backperformance_data(cur, strategy_name, cash, start_date, end_date, final_value, CAGR, ProfitFactor, PayoffRatio, SharpeRatio, Calmar):
    cur.execute("SELECT * FROM backperformance WHERE Strategy = %s AND cash = %s AND start_date = %s AND end_date = %s", (strategy_name, cash, start_date, end_date,))
    existing_row = cur.fetchone()

    if existing_row:
        # If it exists, update the entry
        cur.execute("UPDATE backperformance SET final_value = %s, CAGR = %s, ProfitFactor = %s, PayoffRatio = %s, SharpeRatio = %s, Calmar = %s WHERE Strategy = %s AND cash = %s AND start_date = %s AND end_date = %s",
                    (final_value, CAGR, ProfitFactor, PayoffRatio, SharpeRatio, Calmar, strategy_name, cash, start_date, end_date))
    else:
        # If it doesn't exist, insert a new entry
        cur.execute("INSERT INTO backperformance (Strategy, cash, start_date, end_date, final_value, CAGR, ProfitFactor, PayoffRatio, SharpeRatio, Calmar) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (strategy_name, cash, start_date, end_date, final_value, CAGR, ProfitFactor, PayoffRatio, SharpeRatio, Calmar))
