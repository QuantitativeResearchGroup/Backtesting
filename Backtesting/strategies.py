...
## Set up Strategy tables
conn = establish_database_connection()
cur = conn.cursor()

create_strategy_table_if_not_exists(cur)
create_strategy_summary_if_not_exists(cur)
create_strategy_summary_LONG_if_not_exists(cur)
...
# Global functions for trade management
...
class MySimpleMovingAverage(bt.Indicator):
    lines = ('mysma',)  # Indicator will have a single line
    params = (('period', 20),)  # Default period value
    
    def __init__(self):
        self.addminperiod(self.params.period)  # Ensure enough data for the given period

    def next(self):
        # Check if enough data is available for the indicator's calculation
        if len(self.data) >= self.params.period:
            self.lines.mysma[0] = bt.indicators.SMA(self.data, period=self.params.period)[0]
            print(f"My SMA value: {self.lines.mysma[0]}")
        else:
            # If there's not enough data, set the moving average value to NaN or None
            self.lines.mysma[0] = float('NaN')  # or None, depending on your preference
...
class Strat1RSIVWAP(bt.Strategy):
 ...
 # Enter a short position
    ...
    # add this to a common/manditory/default characteristics for all methods
    def store_trade(self, action, price):  # make this a public method. Inheritence 
        date = self.data.datetime.datetime(0)
        ticker = self.data._name
        quantity = self.position.size
        self.__class__.trade_history.append({
...
class Strat5MACross(bt.Strategy):
    params = (
        ("slow", 200),
        ("fast", 50),
        ("period", 3),
    ...
    def __init__(self):
        pass  # Add any additional initialization logic here if needed

    def ma_cross_strategy(self, end=None):
        ticker = self.data._name  # Get the ticker from the data feed's name

        if not end:
            end = dt.date.today()
        start = end - relativedelta(years=self.params.period)

        data = pd.DataFrame(yf.download(ticker, start=start, end=end)["Close"])
        data[f'{self.params.fast}-day'] = data.Close.rolling(self.params.fast).mean()
        data[f'{self.params.slow}-day'] = data.Close.rolling(self.params.slow).mean()
        data['returns'] = np.log(data.Close).diff()
        data['strategy'] = np.where(data[f'{self.params.fast}-day'] > data[f'{self.params.slow}-day'], 1, 0)
        data['strategy'] = np.where(data[f'{self.params.fast}-day'] < data[f'{self.params.slow}-day'], -1, data['strategy'])
        strategy = data['returns'] * data['strategy']
        return strategy
    

class MaCrossStrategy(bt.Strategy):
    Description = "This strategy is based on moving average crossover. It buys when the shorter moving average crosses above the longer moving average and sells when the opposite occurs."

    params = (
        ('slow', 200),  # Period for slow moving average
        ('fast', 50),   # Period for fast moving average
        ('period', 3),  # Number of years for historical data
    )
    ...
    def __init__(self):
        ...
        self.fast_ma = bt.indicators.SimpleMovingAverage(data.close, period=self.params.fast)
        self.slow_ma = bt.indicators.SimpleMovingAverage(data.close, period=self.params.slow)
