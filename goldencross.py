    ...
    def next(self):
        if self.position.size == 0:
            if self.crossover >0:
                ...
                print("Buy {} shars of {} at {}".format(self.size, self.params.ticker,self.data.close[0]))
                
                self.buy(size=self.size)
                
        if self.position.size >0:
            if self.crossover <0:
                print("sell {} shars of {} at {}".format(self.size, self.params.ticker,self.data.close[0]))
                self.close()
